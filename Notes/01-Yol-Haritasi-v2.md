...
## Faz 8 — UUID + entity referansları
...
- [ ] `UUID` sınıfı (64-bit rastgele, `std::random_device` tohumlu)
- [ ] `IDComponent` — her entity'ye `CreateEntity`'de otomatik eklenir
- [ ] `Scene::FindEntityByUUID(UUID)` + UUID → entity haritası
- [ ] Serileştirmeye UUID yaz, yüklerken **aynı** UUID ile oluştur
- [ ] `Entity` referansları için `EntityRef` (UUID tutar, geç çözümlenir)

**Öğrenilecek:** Kalıcı kimlik ile çalışma zamanı kimliğinin farkı.
Neden hemen hemen her ciddi motorda ikisi ayrıdır.

**Test:** Sahneyi kaydet, yükle, `m_PlayerEntity`'yi UUID ile geri bul —
temizlemeye gerek kalmamalı.
